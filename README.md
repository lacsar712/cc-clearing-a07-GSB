...
单币种多边轧差清算全栈演示：录入义务 → 执行轧差 → 查看净头寸 → 结算指示预览/释放 → 确认 settle。
...
| 用户名 | 密码 | 权限 |
|--------|------|------|
| operator | op123456 | 可写（轧差、结算指示释放、settle、新建会员/义务） |
| viewer | view123456 | 只读（可浏览、可生成结算指示预览，不能释放） |
...
8. 进入「结算指示」页（或批次详情的"结算指示预览 / 释放"入口），选择 COMPLETED 批次生成预览，确认能看到每个会员与金额（应收/应付）
9. 操作员点击「释放指示」，状态由"已预览"变为"已释放"（行高亮），并记录释放人与释放时间；未完成批次不能生成预览，未预览不能释放
10. 使用 `viewer` 登录，确认可浏览并生成预览，但无法释放或执行其他写操作
