...
8. 侧边栏进入「结算指示」页，找到该 COMPLETED 批次（未完成批次不能生成预览）
9. 进入批次指示详情，点击「生成指示预览」，核对每个会员的应付/应收方向与金额（零净额头寸不产生指示）
10. 操作员点击「释放结算指示」，确认后指示状态变为「已释放」，页面保留释放时间与释放人；重复预览幂等、重复释放安全
11. 使用 `viewer` 登录，确认只能浏览与生成预览、无法执行释放等写操作
